...
// chi phí thấp nhất
int a_star(int start, int goal, const unordered_map<int, vector<Edge>> &graph,
  ...
  while (!openSet.empty()) {
    ...
    if (current == goal) {
      return g_score[goal]; // Trả về chi phí ngắn nhất
    }
...
void read_graph_from_file(const string &filename, int &start, int &goal,
...
int main() {
  ...
  if (min_cost != -1) {
    cout << "Chi phí thấp nhất từ " << start << " đến " << goal
         << " là: " << min_cost << endl;
  } else {
